...
const mockPassableTrackPath = path.resolve(__dirname, '..', 'mocks', 'tracks', 'sample.mp3');
...
describe('Testing GET /api/tracks/audio?path=', () => {
  it('should return a cover image of a provided song', async () => {
    const response = await request(app).get(`/api/tracks/audio`).query({
      path: mockPassableTrackPath,
    });
